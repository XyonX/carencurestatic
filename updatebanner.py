...
new_code_file_name = "zzz.html"  # File containing the new <div> code
...
def process_html_file(html_file_path):
    try:
        ...
        new_div = BeautifulSoup(new_code_html, "html.parser").find("div", id="logo")

        # Find the existing <div> with id="logo" in the original HTML
        old_div = soup.find("div", id="logo")

        # Replace the old <div> with the new <div>
        if old_div and new_div:
            old_div.replace_with(new_div)
            print(f"Replaced 'logo' div in {html_file_path}")
        elif not old_div:
            print(f"Warning: No div with id 'logo' found in {html_file_path}")
        elif not new_div:
            print(f"Warning: No div with id 'logo' found in {new_code_file_name}")
